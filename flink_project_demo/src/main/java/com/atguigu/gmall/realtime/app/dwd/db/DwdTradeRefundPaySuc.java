 ...
 * 执行流程：
 *      1、创建动态表，获取 topic_db 中的数据
 *      2、过滤 refund_payment 中的数据，创建动态表
 *          （插入时 refund_status 为【0701审核中】 ，经过【0702商家审核通过】，然后是【0704 买家发货】 ，到最后【0705 退单成功】
 *              /如果【0703 商家审核未通过】，那么最后就是 【0706 退单失败】
 *      3、过滤 order_refund_info 中的数据，创建动态表
 *           当退款完成，refund_type会 显示为 ： 1501 仅退款 / 1502 退货退款
 *      4、过滤 order_info 中的数据，创建动态表
 *            当 发起退款的时候， 会将  order_status --> 【1005 退款中】
 *            退款完成， refund_payment 中的refund_status=【0705 退单成功】，  order_status --> 【1006 退款完成】
 *      5、通过 mysql LoopUp 查询缓存的方式，读取 base_dic 中的数据
 */
public class DwdTradeRefundPaySuc {
    public static void main(String[] args) {
        ...
        tableEnv.executeSql(MyKafkaUtil.getTopicDbTable("dwd_trade_order_refund"));
        ...
        tableEnv.executeSql("select * from dwd_trade_refund_pay_suc").print();
    }
}
